...
def run_real(names,to_html=False, to_print=False,func=Tim):
    ...
    final_df = pd.DataFrame(columns=["N","za", "za err", "zb",
                                "zb err", "alpha", "alpha err", "rchi", "pearson r","pearson p-val",
    ...
    for i in pbar:
        ...
        try:
            ...
            x_err = 1#np.sqrt(pcov[2][2])
            plots = Plotter(names[i])
            ks, inv_c_mean, errs, stds, counts   = unpack_stat_dict(statistics_dict)
            plots.add_plot(ks,inv_c_mean,yerr=errs,fitline=True,function=func,popt=popt)
            ...
            plots_unag.add_plot(k,1/c,fitline=True,function=func,popt=popt)  
            ...
            inv_c_pred = func(ks,popt[0],popt[1])
            ...
            temp_df = pd.DataFrame({"N": len(g.get_vertices()), "za": a, "za err": a_err, 
                                "zb": b, "zb err": b_err,"alpha":x,"alpha err":x_err, "rchi": rchi, "pearson r": r,
