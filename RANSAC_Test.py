...
if __name__ == '__main__':
    ...
    for group in unique_partition:
        plt.plot(k[spectral_labels == group], 1/c[spectral_labels == group], '.', alpha=0.2, label='Cluster ' + str(group))
    plt.legend()
    ...
    plt.errorbar(ks, inv_c_mean, yerr=errs, fmt='v', color='black',ecolor='black' , capsize=2, elinewidth=1, markersize=3, markeredgewidth=0.5, mfc='none',label='Whole Network Mean')
    z_groups = []

    for i in range(1):
        ransac = RANSACRegressor(residual_threshold = 1.63,max_trials=1000000, stop_probability=0.9999999)
        ...
        plt.errorbar(temp_ks, temp_inv_c_mean, yerr=temp_errs, fmt='o', color='blue', capsize=2, elinewidth=0.5, markersize=3, mfc='none', markeredgewidth=0.5, label='Inliers Mean')
    ...
    temp_ks, temp_inv_c_mean, temp_errs, temp_stds, temp_counts   = pb.unpack_stat_dict(stat_dict)
    plt.errorbar(temp_ks, temp_inv_c_mean, yerr=temp_errs, fmt='o', color='red',ecolor='black' , capsize=2, elinewidth=0.5, markersize=3, markeredgewidth=0.5,mfc='none', label='Outliers Mean')
