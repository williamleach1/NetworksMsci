...
def custom_formatter(x):
    coeff, exponent = f"{x:.2e}".split("e")
    return f"{coeff} x 10$^{{{int(exponent)}}}$" if x != 0 else "0"


def calculate_edge_densities(g, partition, test=False):
    n = g.num_vertices()
    m = g.num_edges()
    ...
    bipartite_remove_nodes = []
    for e in g.edges():
        source, target = int(e.source()), int(e.target())
        if partition[source] == 0 and partition[target] == 0:
            intra_edges_0 += 1
            if test:
                bipartite_remove_nodes.append(source)
                bipartite_remove_nodes.append(target)
    ...
    max_intra_edges_0 = nodes_group_0 * (nodes_group_0 - 1) / 2
    max_intra_edges_1 = nodes_group_1 * (nodes_group_1 - 1) / 2
    max_inter_edges_01 = nodes_group_0 * nodes_group_1
    ...
    density_1_0 = inter_edges_01 / max_inter_edges_01

    edge_matrix = np.array([
        [intra_edges_0, inter_edges_01],
        [inter_edges_01, intra_edges_1],
    ])

    # Create an extended matrix with the additional values
    matrix = np.array([
        [density_0_0, density_0_1],
        [density_0_1, density_1_1],
    ])

    if test:
        return matrix, edge_matrix, bipartite_remove_nodes
    else:
        return matrix, edge_matrix
...
if __name__ == '__main__':
    ...
    try:
        ...
        spectral_labels = spectral_clustering.fit_predict(combined_matrix)#combined_matrix)
    ...
    for v in g.vertices():
        v_group[v] = partition[int(v)]
    
    g.vertex_properties['inlier'] = v_group

    inlier = g.vertex_properties['inlier']
    '''
    gt.graph_draw(g, pos=pos, vertex_fill_color=inlier,output='ReportPlots/spectral_result.png')
    '''
    ...
    plt.show()
    '''
    density, extended_matrix  = calculate_edge_densities(g, partition)

    # Extract the edge densities and other metrics
    edge_densities = density
    
    # Visualize the edge densities using Seaborn's heatmap
    '''
    sns.set()
    plt.figure(figsize=(2, 2))
    ax = sns.heatmap(edge_densities, cmap='coolwarm', square=True, annot=False, cbar=False,
                    xticklabels=['Group 0', 'Group 1'], yticklabels=['Group 0', 'Group 1'])

    # Annotate the heatmap with custom formatted text
    # Annotate the heatmap with custom formatted text
    for i in range(edge_densities.shape[0]):
        for j in range(edge_densities.shape[1]):
            ax.text(j + 0.5, i + 0.5, custom_formatter(edge_densities[i, j]),
                    horizontalalignment='center', verticalalignment='center', color='white', fontsize=8)
    ax.tick_params(axis='both', which='major', labelsize=8)
    ax.tick_params(axis='both', which='minor', labelsize=8)
    #plt.title('Edge Densities')
    #plt.xlabel('Source Group', fontsize=8)
    #plt.ylabel('Target Group', fontsize=8)
    plt.subplots_adjust(left = 0.15, right = 0.95, top = 0.95, bottom = 0.15)
    plt.savefig('ReportPlots/edge_densities'+name+'.png', dpi=900)
    plt.close()
    '''
    nodes_group_0 = np.sum (partition == 0)
    nodes_group_1 = np.sum (partition == 1)
    ...
    cols = ['r', 'b', 'g', 'y', 'm', 'c', 'k']
    point_colour = ['pink', 'lightskyblue', 'lightgreen', 'lightyellow', 'violet', 'lightcyan', 'lightgrey']
    line_fmt =['--', '-.', ':', '-']
    for group in unique_partition:
        plt.plot(k[spectral_labels == group], 1/c[spectral_labels == group], '.', alpha=0.7, label='Group ' + str(group), color=point_colour[int(group)])
        popt, pcov = pb.fitter(k[spectral_labels == group], 1/c[spectral_labels == group], pb.Tim)
        statistics_dict = pb.aggregate_dict(k[spectral_labels == group], 1/c[spectral_labels == group])
        rchi = pb.red_chi_square(k[spectral_labels == group], 1/c[spectral_labels == group], pb.Tim, popt, statistics_dict)
        print('Group: ', group, 'rchi: ', rchi, 'popt: ', popt)
        temp_ks, temp_inv_c_mean, temp_errs, temp_stds, temp_counts   = pb.unpack_stat_dict(statistics_dict)
        plt.errorbar(temp_ks, temp_inv_c_mean, yerr=temp_errs, fmt='o', capsize=1.5, elinewidth=0.5,mfc='none',mew=0.4, markersize=3,capthick=0.5, label='Aggregated Data group ' + str(group), color=cols[int(group)])
        plt.plot(temp_ks, pb.Tim(temp_ks, *popt), line_fmt[int(group)], label='Fit group ' + str(group), linewidth=1, color = 'k')
        inv_ln_z = popt[0]
        ...
        print('z_fit_group '+str(group) +' = ', z_fit_outliers)
        print('z_fit_group '+str(group) +' err = ', z_fit_outliers_err)

    plt.legend(markerscale=2, fontsize=12)
    plt.xlabel(r'$k$', labelpad=0, fontsize=14)
    plt.ylabel(r'$\dfrac{1}{c}$', rotation=0, labelpad=15, fontsize=14)
    ymin, ymax = plt.ylim()
    plt.ylim(ymin, 7)
    plt.xscale('log')
    plt.subplots_adjust(left = 0.15, right = 0.98, top = 0.98, bottom = 0.15)
    plt.savefig('ReportPlots/fit'+name+'.png', dpi=900)
    plt.close()
    ...
    for i in range(1):
        inlier_mask = np.asarray(partition)
    ...
    #gt.graph_draw(g, pos=pos, vertex_fill_color=inlier,output='ReportPlots/ransac_result.png', output_size=(2000, 2000))
