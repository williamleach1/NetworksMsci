...
for i in range(len(nodes)):
    for j in range(len(layers)):
        if nodes[i] in layers[j]:
            node_colours.append(all_cols[j])
            node_label[nodes[i]] = 'Ring' + str(j)
            break
...
for i in range(len(layers)):
    ...
    if i > 0 and i < len(layers) - 1:
        ...
        for j in range(len(layers[i])):
            source = layers[i][j]
            # flatten list of nodes in layer above, below, and same layer
            targets = layers[i-1] + layers[i] + layers[i+1]
...
for i in range(len(layers)):
    ax.scatter([], [], c=all_cols[i],s=200, label='Ring ' + str(i))
...
plt.savefig('NetworkDemoPlots/NetworkRingPlot.png', dpi = 1200, bbox_inches = 'tight')
plt.show()
